...
def tf_dqn(I, drones_coverage, folder_name, deployment, packet_update_loss, packet_sample_loss, periodicity, adj_matrix, tx_rx_pairs, tx_users):  
    ...
    print(f"\n\nDQN started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users} and {deployment} deployment")
    print(f"\n\nDQN started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users} and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
...
## https://github.com/tensorflow/agents/blob/755b43c78bb50e36b1331acc9492be599997a47f/tf_agents/agents/dqn/dqn_agent.py#L113
    ...
    if agent.train_step_counter.numpy()==0:
        # time.sleep(10)
        # pass
        print(f"\nDQN scheduling and {deployment} placement with {I} users, coverage is {train_py_env.act_coverage}, DL_capacity is {train_py_env.DL_capacity}, UL_capacity = {train_py_env.UL_capacity}, action space size is {train_py_env.action_size} and they are {train_py_env.actions_space} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
        print(f"\nDQN scheduling and {deployment} placement with {I} users, coverage is {train_py_env.act_coverage}, DL_capacity is {train_py_env.DL_capacity}, UL_capacity = {train_py_env.UL_capacity}, action space size is {train_py_env.action_size} \n\n", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    pickle.dump(eval_py_env.dest_age, open(folder_name + "/" + deployment + "/" + str(I) + "U_DQN_dest_age.pickle", "wb")) 

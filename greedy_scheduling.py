...
def find_greedy_action(eval_env): ## maximal age difference
    ## https://pynative.com/python-random-seed/
    seedValue = random.randrange(sys.maxsize)
    random.seed(seedValue)
    
    # print(f"eval_env.tx_rx_pairs = {eval_env.tx_rx_pairs}")
    ## the age is the difference of a user's age at the destination node and its age at the BS        greedy_age_dest = {x:eval_env.dest_age[x] for x in eval_env.tx_rx_pairs}
    greedy_age_dest = {tuple(x):eval_env.dest_age[tuple(x)] for x in eval_env.tx_rx_pairs}
    
    if verbose:
        print(f"age of users at dest is {greedy_age_dest} where age at dest is {eval_env.dest_age}, age at UAV is {eval_env.UAV_age}")
        print(f"greedy_age_dest = {greedy_age_dest}")
   
    ## pair selection for updating begins

    sampleDict_copy = copy.deepcopy(greedy_age_dest)
 ...
 ############################################
    ...
    greedy_action = list(itertools.product([sampled_users], [updated_users]))
        
    ## finally collect the result and compare with the action space of the environment
    updated_users = sorted(greedy_action[0][1]) ## doesn't affect so keep it. but if due to this 11-10 and 10-11 are same, wrong
    sampled_users = sorted(greedy_action[0][0])
    
    actual_greedy_action = None # this has to change
    ...
    for action in range(eval_env.action_size):
        ...
        if sorted(list(eval_action[0]))==sorted(sampled_users) and sorted(list(eval_action[1]))==sorted(updated_users):
            actual_greedy_action = action
            break

    if verbose:
        print(f"updated_users are {updated_users}, sampled_users = {sampled_users}, greedy_action = {greedy_action}, actual_greedy_action = {actual_greedy_action}")
        
    assert actual_greedy_action!=None
        
    return actual_greedy_action    
...
def greedy_scheduling(I, drones_coverage, folder_name, deployment, packet_update_loss, packet_sample_loss, periodicity, adj_matrix, tx_rx_pairs, tx_users):  ## greedy 
    ...
    for ep in range(random_episodes): # how many times this policy will be run, similar to episode
        ...
        if ep==0:
            print(f"\ngreedy scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, DL_capacity is {eval_env.DL_capacity}, UL_capacity = {eval_env.UL_capacity}, action space size is {eval_env.action_size} and they are {eval_env.actions_space} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            print(f"\ngreedy scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, DL_capacity is {eval_env.DL_capacity}, UL_capacity = {eval_env.UL_capacity}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        for x in range(MAX_STEPS):
            ...
            selected_action = find_greedy_action(eval_env)
    ...
    pickle.dump(UAV_returns, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_UAV_returns.pickle", "wb"))
    pickle.dump(age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_age_dist_UAV.pickle", "wb"))
    pickle.dump(age_dist_dest, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_age_dist_dest.pickle", "wb"))
    
    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_time.pickle", "wb"))

    pickle.dump(attempt_sample, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_attempt_sample.pickle", "wb"))
    pickle.dump(success_sample, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_success_sample.pickle", "wb"))
    pickle.dump(attempt_update, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_attempt_update.pickle", "wb"))
    pickle.dump(success_update, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_success_update.pickle", "wb"))
    
    
    print("\ngreedy scheduling ", deployment, " placement, ", I, " users - avg of final_step_rewards = ", np.mean(final_step_rewards), " MIN and MAX of final_step_rewards = ", np.min(final_step_rewards),", ", np.max(final_step_rewards), " and avg of overall_ep_reward = ", np.mean(overall_ep_reward), " : end with final state of ", eval_env._state, " with shape ", np.shape(eval_env._state))
    
    print("\ngreedy scheduling ", deployment, " placement, ", I, " users - avg of final_step_rewards = ", np.mean(final_step_rewards), " MIN and MAX of final_step_rewards = ", np.min(final_step_rewards),", ", np.max(final_step_rewards), " and avg of overall_ep_reward = ", np.mean(overall_ep_reward), " : end with final state of ", eval_env._state, " with shape ", np.shape(eval_env._state), file = open(folder_name + "/results.txt", "a"), flush = True)
